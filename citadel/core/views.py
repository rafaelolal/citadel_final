...
class MaxProfitView(APIView):
    def get(self, request, format=None):
        ...
        data = ticker.get_historical_price_data(start, end, "daily")
        prices = [day["close"] for day in data[ticker_symbol]["prices"]]
        dates = [
            day["formatted_date"] for day in data[ticker_symbol]["prices"]
        ]
        ...
        for metric, function in metrics_functions.items():
            ...
            profit = max_profit(
                prices,
                dates,
                decision_functions[metric](prices, metric_values),
            )
            data[metric] = profit
